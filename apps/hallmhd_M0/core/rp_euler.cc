         ...
         FArray<double>& amdq, FArray<double>& apdq)
{
    ...
    for(int i=2-mbc; i<=mx+mbc; i++)
    {
        ...
        {
            std::cout << "*** Negative density in routine rp in file rp_euler.cc" << std::endl;
        ...
        hv->u(i) = (qr(i-1,2)/rhsqrtl + ql(i,2)/rhsqrtr) / rhsq2;
        hv->v(i) = (qr(i-1,3)/rhsqrtl + ql(i,3)/rhsqrtr) / rhsq2;
        hv->w(i) = (qr(i-1,4)/rhsqrtl + ql(i,4)/rhsqrtr) / rhsq2;

        hv->u2v2w2(i) = pow(hv->u(i),2) + pow(hv->v(i),2) + pow(hv->w(i),2);

        // Roe-averaged enthalpy
        hv->enth(i) = (((qr(i-1,5)+pl)/rhsqrtl
                        + (ql(i,5)+pr)/rhsqrtr)) / rhsq2;

        // speed of sound
        a2 = gas_gamma1*(hv->enth(i) - .5*hv->u2v2w2(i));
        hv->a(i) = sqrt(a2);
        ...
        hv->g1a2(i) = gas_gamma1 / a2;
        hv->euv(i)  = hv->enth(i) - hv->u2v2w2(i);
    }
    ...
    for(int i=2-mbc; i<=mx+mbc; i++)
    {
        ...
        a4 = hv->g1a2(i) * (hv->euv(i)*delta(1)
                            + hv->u(i)*delta(2) 
                            + hv->v(i)*delta(3) 
                            + hv->w(i)*delta(4)
                            - delta(5));
        a2 = delta(3) - hv->v(i)*delta(1);
        a3 = delta(4) - hv->w(i)*delta(1);
        a5 = (delta(2) + (hv->a(i)-hv->u(i))*delta(1) - hv->a(i)*a4) / (2.0*hv->a(i));
        a1 = delta(1) - a4 - a5;

        // Wave 1: eigenvalue is u-c
        wave(i,1,1)  = a1;
        wave(i,2,1) = a1*(hv->u(i)-hv->a(i));
        wave(i,3,1) = a1*hv->v(i);
        wave(i,4,1) = a1*hv->w(i);
        wave(i,5,1)  = a1*(hv->enth(i) - hv->u(i)*hv->a(i));
        s(i,1) = hv->u(i)-hv->a(i);

        // Wave 2: the 3 eigenvectors corresponding to the repeated
        // eigenvalue u,u,u are lumped together into a single wave
        wave(i,1,2)  = a4;
        wave(i,2,2) = a4*hv->u(i);
        wave(i,3,2) = a4*hv->v(i)	 	 + a2;
        wave(i,4,2) = a4*hv->w(i)	 	 + a3;
        wave(i,5,2)  = a4*0.5*hv->u2v2w2(i)  + a2*hv->v(i) + a3*hv->w(i);
        s(i,2) = hv->u(i);

        // Wave 3: eigenvalue is u+c
        wave(i,1,3)  = a5;
        wave(i,2,3) = a5*(hv->u(i)+hv->a(i));
        wave(i,3,3) = a5*hv->v(i);
        wave(i,4,3) = a5*hv->w(i);
        wave(i,5,3)  = a5*(hv->enth(i)+hv->u(i)*hv->a(i));
        s(i,3) = hv->u(i)+hv->a(i);
    }
