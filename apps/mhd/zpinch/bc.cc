...
bc(const Run_Data& rd, FArray<double>& q)
{
    ...
    {
        // this is an axis at the left
        // copy everything to ghost cells
        for(int ibc=1; ibc<=mbc; ibc++)
            for(int m=1; m<=meqn; m++)
                for(int cc=1; cc<=ncoeffs; cc++)
                    q(1-ibc,m,cc) = q(ibc,m,cc);

        // r-momentum
        for(int ibc=1; ibc<=mbc; ibc++)
            q(1-ibc,2,1) = -q(1-ibc,2,1);
        // phi-momentum
        for(int ibc=1; ibc<=mbc; ibc++)
            q(1-ibc,3,1) = -q(1-ibc,3,1);
        // B-r
        for(int ibc=1; ibc<=mbc; ibc++)
            q(1-ibc,6,1) = -q(1-ibc,6,1);
        // B-phi
        for(int ibc=1; ibc<=mbc; ibc++)
            q(1-ibc,7,1) = -q(1-ibc,7,1);
    }
