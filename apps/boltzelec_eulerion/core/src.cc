...
src(const Run_Data& rd, FArray<double>& sr, FArray<double>& q)
{
    ...
    for (int i=1; i<=mx; i++)
    {
        ...
        sr(i,1) = 0.0;
        sr(i,2) = qbym*(ev->ef(i,1) +rhov*ev->bf(i,3) -rhow*ev->bf(i,2));
        sr(i,3) = qbym*(ev->ef(i,2) +rhow*ev->bf(i,1) -rhou*ev->bf(i,3));
        sr(i,4) = qbym*(ev->ef(i,3) +rhou*ev->bf(i,2) -rhov*ev->bf(i,1));
        sr(i,5) = rhou*ev->ef(i,1)  +rhov*ev->ef(i,2) +rhow*ev->ef(i,3);
